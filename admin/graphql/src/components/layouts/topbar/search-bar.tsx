  ...
  return (
    <Fragment>
      ...
      <div className="relative w-full max-w-lg rounded-3xl">
        <SearchIcon className="absolute inset-y-0 w-4 h-4 my-auto left-4" />
        <input
          type="text"
        ...
        {!isEmpty(searchItem) && (
          <button
            className="absolute w-auto h-auto px-0 text-sm font-medium text-gray-500 -translate-y-1/2 top-1/2 hover:text-accent-hover ltr:right-4 rtl:left-4"
            onClick={(e) => {
              e.preventDefault();
              setSearchText('');
            }}
              ...
              <div className="mx-3">
                {searchItem?.map((item) => {
                  return (
                    ...
                    >
                      <span className="inline-flex items-center justify-center p-2 text-gray-500 border border-gray-200 rounded-md shrink-0 group-hover:border-gray-300">
                        <TermsIcon className="w-5 h-5" />
                      </span>
